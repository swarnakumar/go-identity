...
var userListingTemplate = templates.Parse("admin/user-listing.html", "components/pagination.html")

func RenderUserListing(s webapp.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		c1 := make(chan []*sqlc.ListUsersRow)
		c2 := make(chan int64)
		...
		go func() {
			...
			listing, err := db.Users.Listing(ctx, offset, int32(limit))
			if err != nil {
		...
		go func() {
			count := db.Users.Count(ctx)
			c2 <- count
		}()

		var listing []*sqlc.ListUsersRow
		var count int64
		...
		s.ExecuteTemplate(w, r, userListingTemplate, &params)
	}
}
